...
## 简介
为复数机器人群聊设计的防止炸群插件。弱黑名单是一种**打断机制**，用于防止多个机器人在群聊中互相触发、反复大量聊天。被加入弱黑名单的账号发送的消息会以一定概率被忽略，而非完全屏蔽——消息仍会被记录到对话历史中，只是不一定会收到回复。插件还支持保底回复机制，确保黑名单用户不会长时间得不到任何回应。适用于多机器人，想看bot们互动的群聊。
...
- **AI 工具调用**：支持通过 LLM 工具调用扫描群内机器人并批量添加弱黑名单
- **关键词硬屏蔽**：可配置消息关键词，命中后直接屏蔽，避免机器人互相识别并回复
...
### AI 工具调用
插件注册了两个 LLM 工具，AI 可以在对话中自动调用：
- `scan_group_bots` - 扫描指定群中名字含特定关键字的疑似机器人账号
- `batch_add_to_blacklist` - 将指定 QQ 号批量添加到弱黑名单

通过工具添加的黑名单会自动同步到 dashboard 配置界面，无需手动操作。例如，输出：‘小贝，检查一下群里的其他机器人，把他们加入一下弱黑名单。’在正确情况下，机器人将自动调用工具，并标记所有名字中带有bot、机器人等关键字的用户。
...
- `blocked_message_keywords`：消息硬屏蔽关键词列表（默认：`["🔨 调用工具:"]`）
  - 消息文本包含任一关键词时会直接停止事件
  - 命中后不进入弱黑名单概率判断，也不会增加保底计数

### 工作原理
1. 收到消息后，插件会先检查是否命中硬屏蔽关键词：
   - 默认关键词为 `🔨 调用工具:`，命中后直接屏蔽，不进入保底计数
2. 当黑名单用户/群聊发送普通消息时，插件会检查是否应该回复：
   - 如果用户已连续被拦截次数达到最大值，触发保底回复
   - 否则根据设定的概率决定是否回复
3. 如果决定不回复，用户消息仍会被处理但不会收到回复，同时拦截计数+1
4. 如果决定回复（概率通过或保底触发），拦截计数重置为0

### 动态黑名单管理
- 通过命令或 AI 工具添加的黑名单会自动同步到 dashboard 配置界面
- 通过 `/rrbot <识别码> remove` 只能移除通过命令添加的黑名单
- 配置文件中直接添加的黑名单需要通过后台管理界面移除
